...
load_tiff_file (std::string const& filename)
{
    ...
    if (!tif){
        ...
        printf("TIFF file format not recognized");
    }
    try
    {
        ...
        if (bits != 8){
            ...
            printf("Expected 8 bit TIFF file");
        }
...
load_tiff_16_file (std::string const& filename)
{
    if (sizeof(uint16_t) != 2){
        std::cerr<<"Need 16bit data type for TIFF image."<<std::endl;
    ...
    TIFF* tif = TIFFOpen(filename.c_str(), "r");
    if (!tif){
        std::cerr<<"TIFF file format not recognized."<<std::endl;
        throw util::Exception("TIFF file format not recognized");
    }
    ...
    try
    {
        ...
        TIFFGetField(tif, TIFFTAG_BITSPERSAMPLE, &bits);
        if (bits != 16){
           std::cerr<<"TIFF file bits per sample don't match"<<std::endl;
